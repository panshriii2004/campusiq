...
/* ─── STATE & CONFIG ─── */
// API URL configuration with /api suffix
...
function selectRole(role, btn) {
    ...
    console.log("Login Role set to:", currentRole);
}
...
async function doRegister() {
    ...
    try {
        ...
        if (response.ok) {
            window.currentUser = data.user;
            showToast(`Account Created as ${registrationRole}! 🎓`, "success");
            enterApp();
        } else {
...
function buildSidebar() {
    ...
    const userInitials = user.name ? user.name.split(' ').map(w=>w[0]).slice(0,2).join('') : '??';
